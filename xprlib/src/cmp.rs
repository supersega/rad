...
use crate::{dual::Dual, xpr::expression::{Xpr, XprWrapper}};
...
impl<T: Xpr> PartialEq<XprWrapper<T>> for Dual {
    fn eq(&self, other: &XprWrapper<T>) -> bool {
        self.eq(&Dual::from(*other))
    }
}

impl<T: Xpr> PartialEq<Dual> for XprWrapper<T> {
    fn eq(&self, other: &Dual) -> bool {
...
impl<T: Xpr> PartialOrd<XprWrapper<T>> for Dual {
    fn partial_cmp(&self, other: &XprWrapper<T>) -> Option<Ordering> {
        self.partial_cmp(&Dual::from(*other))
    }
}

impl<T: Xpr> PartialOrd<Dual> for XprWrapper<T> {
    fn partial_cmp(&self, other: &Dual) -> Option<Ordering> {
